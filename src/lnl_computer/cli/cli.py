...
from .main import batch_lnl_generation, combine_lnl_data, make_mock_obs, make_sf_table
...
def cli_make_sf_table(
    ...
    """
    make_sf_table(parameters=parameters, n=n, grid_parameterspace=grid_parameterspace, fname=fname)


@click.command(name="make_mock_obs")
@click.argument('compas_h5_path', type=str)
@click.argument('sf_sample', type=str,
                default="aSF:0.01 dSF:0.01 muz:0.01 sigma0:0.01")
@click.option('--fname', type=str, help='Output filename', default='mock_observation.npz')
...
@click.argument('mcz_obs', type=click.Path(exists=True))  # Assuming the path to the npz file
@click.argument('compas_h5_path', type=str)
@click.argument('parameter_table', type=click.Path(exists=True, dir_okay=False))  # Assuming it's a CSV file
@click.option('--n_bootstraps', default=100, help='Number of bootstraps to generate for each parameter set', type=int)
@click.option('--save_images/--no_save_images', default=True, help='Save images of the generated mcz-grids',
              is_flag=True)
@click.option('--outdir', default="out_mcz_grids", help='Output directory for mcz-grids', type=str)
def cli_batch_lnl_generation(
    ...
    :return: None
    """
    batch_lnl_generation(mcz_obs, compas_h5_path, parameter_table, n_bootstraps, save_images, outdir)
